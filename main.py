...
def verificar_libro(title: str, author: str):
    """Verifica si un libro específico está en la lista."""
    data = cargar_data()
    for libro in data:
        if libro.get("title") == title and libro.get("author") == author:
            return {"message": f"📖 El libro '{title}' de '{author}' está en la lista."}
    raise HTTPException(status_code=404, detail=f"No se encontró el libro '{title}' de '{author}'.")
...
def crear_libro(nuevo_libro: Libro):
    ...
    guardar_data(data)
    return {"message": "✅ Libro agregado exitosamente.", "Libro": nuevo_libro_dict}
...
def modificar_libro(title: str, author: str, libro_actualizado: Libro):
    ...
    for idx, libro in enumerate(data):
        if libro.get("title") == title and libro.get("author") == author:
            ...
            guardar_data(data)
            return {"message": f"✏️ Libro '{title}' de '{author}' modificado con éxito.", "Libro": libro_actualizado}
    raise HTTPException(status_code=404, detail=f"No se encontró un libro con título '{title}' y autor '{author}'.")


@app.delete("/delete-book")
def eliminar_libro(title: str, author: str):
    """Elimina un libro del JSON por título y autor."""
    data = cargar_data()
    for libro in data:
        if libro.get("title") == title and libro.get("author") == author:
            data.remove(libro)
            guardar_data(data)
            return {"message": f"🗑️ Libro '{title}' de '{author}' eliminado correctamente."}
